...
ES.deploy = '${DEPLOY}'
ES.releaseContainerDomain = '${RELEASECONTAINERDOMAIN}'
ES.haoxiangDomain = '${HAOXIANGDOMAIN}'
ES.tracking = {
	url: '${TRACKINGURL}',
	key: '${TRACKINGKEY}'
}
...
ES.util.isMobilePhone = function (str) {
	if(!str){
		return false
	}
	str = str.toString()
...
ES.util.validation_is_float_num = function (es_input, number) {
	if (es_input.get_value() != '') {
		if (es_input.get_value() * 1 < 0) {
			es_input.set_msg(ES.msg.more_than_zero)
			return false;
		...
		if (val.substring(val.lastIndexOf(".")).length < 3) {
			val = (es_input.get_value() * 1).toFixed(number || 3)
		}
		...
		if (!result) {
			switch (number){
				...
				case 3:
					...
					break
				case 4:
					es_input.set_msg(ES.msg.reg_isFloatNumFour_fail)
					break
				case 5:
					es_input.set_msg(ES.msg.reg_isFloatNumFive_fail)
					break
			}
		}
		return result;
...
ES.util.validation_plate_number = function (es_input) {
	if (es_input.get_value() != '') {
		...
		if (!result) {
			es_input.set_msg(ES.msg.get('reg_plate_number'))
		}
		return result;
	}
	return true
		...
		success: function (res) {
			if (res.message) {
				...
				if(res.status == 'FAILED'){
					ES.ui.alert({
						key: ES.msg.get('upload_failure',{msg: res.data || res.message}),
						type: 'error'
					})
				} else {
					callback(res)
...
ES.util.login = function (hasBack) {
	if (ES.sso_truck.loginType === "sso") {
		window.location.href = ES.sso_truck.login + (hasBack ? encodeURIComponent("?returl="+window.location.href) : "");
	} else if (ES.sso_truck.loginType === "portx") {
		window.location.href = 'http://www.portx.cn/bsplatform/login/index.do';
	}else {
		//hasBack ? ES.util.forwardredirect('truck-login.html') : ES.util.redirect('truck-login.html')
		ES.util.redirect('truck-home.html')
	}
}
ES.util.forwardRedirectLogin = function () {
	ES.util.login(true)
}
ES.util.redirectLogin = function () {
	ES.util.login(false)
}
...
ES.show = function () {
	...
	var initI18N = function () {
		...
		ES.get('.left-products a').eq(7).html(ES.msg.get('foreign_LCL'))
		// ES.get('.left-products a').eq(7).html('')
		ES.get('.left-products a').eq(8).html(ES.msg.get('eshipping_express'))
		ES.get('.left-products a').eq(9).html(ES.msg.get('eshipping_uexpress'))
		ES.get('.left-products a').eq(10).html(ES.msg.get('trucking_fee'))
		...
		if (ES.lang != ES.consts.language.ZH_CN) {
			...
			ES.get('.left-products').eq(9).show()
 ...
 * unit : 单位符号
 * decimalCount: 小数位数
 * ignoreDecimal: 如果为true, 不考虑小数，即传进来几位小数就保留几位小数
 *
 * demo:
 * ES.util.formatDataByUnit({value: 123456, unit: 'CNY'})   return 'CNY 123,456.00'
 * ES.util.formatDataByUnit({value: 123456.1, unit: 'CNY',ignoreDecimal: true})   return 'CNY 123,456.1'
 * */
ES.util.formatDataByUnit = function(config) {
	config.unit = config.unit !== undefined ? config.unit + ' ' : "CNY ";
	config.seperator = config.seperator || ",";
	config.decimal = config.decimal || ".";
	if(config.ignoreDecimal) {
		config.value = config.value + ''
		config.decimalCount = config.value.indexOf(config.decimal) > -1 ? config.value.length - config.value.indexOf(config.decimal) - 1 : 0
	} else {
		config.decimalCount = !isNaN(config.decimalCount = Math.abs(config.decimalCount)) ? config.decimalCount : 2;
	}
    var number = config.value,
        negative = number < 0 ? "-" : "",
        i = parseInt(number = Math.abs(+number || 0).toFixed(config.decimalCount), 10) + "",
        j = (j = i.length) > 3 ? j % 3 : 0;

    return config.unit + negative + (j ? i.substr(0, j) + config.seperator : "") + i.substr(j).replace(/(\d{3})(?=\d)/g, "$1" + config.seperator) + (config.decimalCount ? config.decimal + Math.abs(number - i).toFixed(config.decimalCount).slice(2) : "")
}
...
ES.util.pinyin = {
	...
	returnCaps: function(value){
		var list = []
		for (var i = 0; i < value.length; i++) {
			var v = value.charAt(i)
			if (v >= 'A' && v <= 'Z') list.push(v)
		}
		return list.join('')
	},
	get: function (l1) {
...
ES.util.authority_redirect = function (authorities) {
	ES.each(ES.consts.authorities, function (_, redirectUrl) {
		var result = false
		ES.each(authorities, function (index, authority) {
			if (redirectUrl == authority) {
				result = true
				ES.util.backdirect(ES.msg.get(redirectUrl))
				return false
			}
		})
		if (result) {
			...
			if (_ == ES.consts.authorities.length - 1) ES.util.backdirect('box-list.html')
		}
	})
}
ES.util.tab_refresh = function (refreshFunction) {
	// 各种浏览器兼容
...
ES.util.controlColumnWidth = function (config){

	this._createGrips = function (config) {
		var div = '<div class="JCLRgrips"></div>'
		var tableTd = ES.get(config.el).parent().find('.sticky-enabled tbody tr:eq(0) td')
		var height = ES.get(config.el).parent().find('.sticky-enabled').height()
		if(!tableTd.length) return

		var verticalBars = []
		ES.each(tableTd, function (index) {
			var left = 0
			for(var i = 0; i< index+1; i++){
				left += ES.get(tableTd[i]).outerWidth()
			}
		...
		ES.get(config.el).parent().prepend('<div class="JCLRgrips">' + verticalBars.join('') + '</div>')

		var mouseEvent = function (e) {
			switch (e.type) {
				case 'mousedown':
					ES.get(this).parent().addClass('dragging')
					ES.get(this).parent().prev().addClass('dragging-before')
					ES.get(document).bind('mousemove', mouseEvent)
					document.onmousedown = function(){return false}
					break
				case 'mouseup':
					var dragging = ES.get('.dragging')
					var index = dragging.index()
					if(!dragging.length) return
					if(!ES.get('.dragging').parents('.admin-table').find('.sticky-enabled tr:last td:eq('+index+')').hasClass('frozen')) {

						var width = 0
						if (index > 0) {
							width = dragging.offset().left - dragging.parent().find('.JCLRgrip:eq(' + (index - 1) + ')').offset().left
						} else {
							width = dragging.offset().left
						}

						ES.each(dragging.parents('.admin-table').find('.sticky-enabled tbody tr'), function (_, itm) {
							ES.get(itm).find('td:eq('+index+')').css({
								'min-width': width,
								'max-width': width
							})
						})

						/*ES.each(ES.get('.dragging').parents('.admin-table').find('.sticky-enabled thead tr:last th'), function (index, item) {
							var width = ES.get(item).outerWidth()
							ES.get('.dragging').parents('.admin-table').find('.sticky-thead thead tr:last th:eq('+index+')').css({
								'min-width': width,
								'max-width': width,
								width: width
							})
					...
					ES.each(ES.get('.dragging').parent().find('.JCLRgrip'), function (index, item) {
						var left = 0
						for (var i = 0; i < index + 1; i++) {
							left += ES.get('.dragging').parents('.admin-table').find('.sticky-enabled tbody tr:eq(0) td:eq('+i+')').outerWidth()
						}
						ES.get(item).css({
							left: left
						})
					...
					ES.get('.dragging').removeClass('dragging')
					ES.get('.dragging-before').removeClass('dragging-before')

					document.onmousedown = function(){return true}
					break
				case 'mousemove':
					if (ES.get('.dragging').length) {
						var left = e.clientX - ES.get('.dragging').parents('.admin-table').find('.sticky-enabled').offset().left
						ES.get('.dragging').css({
							left: left
						})
					}
		...
		ES.get(config.el).parent().find( '.JColResizer').on('mousedown', mouseEvent)
		ES.get(document).unbind('mouseup').on('mouseup', mouseEvent)


		ES.each(ES.get('.admin-table').find('.sticky-enabled thead tr:last th'), function (index, item) {
			var width = ES.get(item).outerWidth()
			ES.get('.dragging').parents('.admin-table').find('.sticky-thead thead tr:last th:eq('+index+')').css({
				'min-width': width,
				'max-width': width,
	...
	this._createGrips(config)

}

ES.util.wheelzoomCanvas = function (config) {

	var canvas = config.canvas
	if(!$.support.leadingWhitespace){
		canvas=window.G_vmlCanvasManager.initElement(canvas);
	}
	var rotateBtn = ES.get(config.canvas).parent().find('.rotate')[0]
	var saveBtn = ES.get(config.canvas).parent().find('.save')[0]
	var enlargeBtn = ES.get(config.canvas).parent().find('.enlarge')[0]
	var narrowBtn = ES.get(config.canvas).parent().find('.narrow')[0]
	...
	img.onload = function () {
		...
		var handleScroll = function (evt) {
			...
			if (evt.type == 'click') {
				...
				if (ES.get(evt.target).hasClass('fa-search-minus')) {
					delta = -3
				} else if (ES.get(evt.target).hasClass('fa-search-plus')) {
					delta = 3
				}
			} else {
